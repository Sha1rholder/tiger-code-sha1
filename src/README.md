...
5. 读取`upstream/ESDB.txt`为`set[Text]`，读取`add/`中参与生成的`.txt`英文附加词典为`list[str]`，逐文件交给`utils/en.py`排序并写回，再生成英文排序和ESDB大小写扩增词
...
ESDB读入为`set[Text]`后扩增大小写形式：若词面全小写，则加入全大写和首字母大写形式；否则加入全大写和全小写形式。附加英文词不会参与扩增；基础排序完成后，主流程只保留长度至少为`MIN_WORD_LEN = 4`且未与附加词完全重复的词，再执行基础词大小写重排
...
英文附加词原样放在`lua/en_dict.txt`顶部，不参与ESDB大小写扩增，也不会被基础词重排移动。基础英文词生成`list[Text]`后会按最终输出顺序拆成基础词、首字母大写词、前两字母大写词三组，再拼接写入`lua/en_dict.txt`：

- 基础词保留在最前
- 首字母大写词排在基础词之后
- 首字母和第二个字母都大写的词排在最后
- 每组内部保持基础英文排序后的原始相对顺序
