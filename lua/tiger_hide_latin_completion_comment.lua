...
---A single space is used as the replacement comment because an empty comment can
---fall back to the original completion hint in Weasel 0.17/librime-lua.
...
local function filter(input, _env)
	for cand in input:iter() do
		if has_latin_letter(cand.text) then
			-- Rebuild the candidate instead of wrapping it, so inherited comments
			-- from table completion entries do not leak through.
