...
---按`tiger.dict.yaml`中的weight重排虎码候选，绕过Rime默认的码长优先排序。
...
local function open_dict()
	...
	return io.open("tiger.dict.yaml", "r")
end

---读取`tiger.dict.yaml`正文，按text缓存所有(code, weight, rank)词条。
local function load_weights()
	...
	if file == nil then
		if log ~= nil and log.warning ~= nil then
			log.warning("tiger_weight_sort: failed to open tiger.dict.yaml")
		end
		return
	end
	...
	if log ~= nil and log.info ~= nil then
		log.info("tiger_weight_sort: loaded " .. count .. " weights")
	end
end
...
local function filter(input, env)
	...
	-- 未在`tiger.dict.yaml`中查到weight的候选保留在末尾，并维持原始相对顺序。
